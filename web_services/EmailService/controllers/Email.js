var nodemailer = require("nodemailer");
// import React, { useEffect, useState } from "react";
// import Axios from "axios";
import Reservations from "../../../client/src/components/Reservations/Reservations";
//pwd - sliit#DSa2
//email -moviereservationsliit@gmail.com
//@desc   Get all reservations
//@route  GET /api/v1/
//@access Public
// exports.sendEmail = async (req, res) => {

// const [reservation, setreservation] = useState([]);

// Axios.get(`${API}api/v1/reservations/tickets/${res.data.id}`).then((res) => {
//   setreservation(res.data.data);
//   console.log(res.data.data);
//   console.log("pabasara");
// });

// const reservation = {
//   movieName,
//   theaterName,
//   noOfTickets,
//   Date,
//   Time,
//   paymentType,
//   totalprice
// } = res.data.data;

var transpoter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "moviereservationsliit@gmail.com",
    pass: "sliit#DSa2",
  },
});

var mailOption = {
  from: "moviereservationsliit@gmail.com",
  to: "moviereservationsliit@gmail.com,pabapalihena@gmail.com",
  subject: "Confirm the movie reservation",
  html: `<style>
      table, th, td {
        border: 1px solid;
      }
      </style>
      <h1>Summary Details of Booking</h1>
      <br/>
      <table>
        <tr>
          <th>Movie</th>
          <th>Theater Name</th>
          <th>No of Tickets</th>
          <th>Date</th>
          <th>Time</th>
          <th>Payment</th>
          <th>Total Price</th>
          <th>Status</th>
          
        </tr>
        <tr>
          <td>${reservation.movieName}</td>
          <td>${theaterName}</td>
          <td>${noOfTickets}</td>
          <td>${Date}</td>
          <td>${Time}</td>
          <td>${paymentType}</td>
          <td>${totalprice}</td>
        </tr>
      </table>
      <br/>
      // <img src="data:image/png;base64," alt="QR">
      <p>Thanks and Regards,<br/>XXXX Online Movie Reservation System</p>`,
};

// attachments: [
//   {
//     filename: "image.png",
//     path: "/path/to/file",
//   },
// ];
transpoter.sendMail(mailOption, function (error, info) {
  if (error) {
    console.log(error);
  } else {
    console.log("Email send info :" + info.response);
  }
});
// };
